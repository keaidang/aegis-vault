...
async def login(request: Request, password: str = Form(...)):
    scope_key = client_address(request)
...
async def logout(request: Request):
    session_id, _ = get_current_session(request)
    session_store.destroy(session_id)
    response = RedirectResponse(url="/", status_code=303)
    clear_session_cookie(response)
    return response
...
async def setup(
    ...
    if len(checkin_code.strip()) < 6:
        return redirect_with_message("签到码至少需要 6 位")
...
async def update_duress_code(request: Request, csrf_token: str = Form(...), duress_code: str = Form(...)):
    require_session(request, csrf_token=csrf_token, admin_only=True)
    if len(duress_code.strip()) < 6:
        return redirect_with_message("胁迫销毁码至少需要 6 位")
