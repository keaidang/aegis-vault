...
**Aegis Vault** 是一个面向高隐私、高胁迫场景的自托管加密保险库。它不是通用网盘，而是一个强调“失联自毁、胁迫销毁、最小暴露面、可审计安全边界”的小型 FastAPI 单体应用。
...
- **Slow Secret Hashing**：签到码与胁迫码使用 `scrypt` 慢哈希，降低离线爆破风险
...
- 签到码 / 胁迫码使用 `scrypt` 慢哈希，旧版裸 `SHA-256` hash 不再兼容
- 私钥、状态文件、HMAC、签到码、胁迫码等敏感文件使用原子写入并收紧权限
- 审计日志链式哈希防篡改
- 文件名规范化与路径穿越防护
...
├── tests/
│   └── test_secret_hashing.py  # 慢哈希与敏感文件权限测试
...
签到码与胁迫码不是明文保存，也不再使用快速 `SHA-256(secret)` 保存。当前版本使用 Python 标准库 `hashlib.scrypt` 生成慢哈希记录：

- 每次设置都会生成独立随机 salt
- hash 文件保存 KDF 参数、salt 与派生结果
- 默认参数可通过 `SECRET_SCRYPT_N`、`SECRET_SCRYPT_R`、`SECRET_SCRYPT_P`、`SECRET_SCRYPT_MAXMEM` 调整
- 旧版 32 字节 `SHA-256` hash 不再兼容，升级后需要重新设置签到码 / 胁迫码

这类设计无法阻止在线猜测，但能显著提高攻击者拿到 `checkin.hash` 或 `duress.hash` 后进行离线爆破的成本。
...
密钥、状态、HMAC、签到码、胁迫码等敏感文件采用临时文件写入、`fsync`、`os.replace` 的原子替换流程，降低半写文件导致状态损坏或误判的概率。

默认权限策略：

- `keys/` 与 `vault/` 目录：`0700`
- 私钥、状态密钥、状态文件、HMAC、签到码 hash、胁迫码 hash：`0600`
- 公钥：`0644`
...
| 离线爆破签到码 / 胁迫码 | `scrypt` 慢哈希 + 随机 salt | 🟢 显著提高离线猜测成本 |
...
| `SECRET_SCRYPT_MAXMEM` | `134217728` | `scrypt` 最大内存预算，单位字节 |

补充说明：

- 前端已登录页面存在固定 `60s` 无操作自动登出机制
- 该 `60s` 计时目前写在前端模板中，尚未独立做成环境变量
- 签到码与胁迫码使用 `scrypt` 慢哈希保存；旧版 `SHA-256` hash 不再兼容，升级后需要重新设置相关口令
...
这次版本的重点是第一轮安全迭代：收紧签到码 / 胁迫码的离线爆破风险，并强化敏感文件写入与权限边界。

#### 安全加固

- 签到码与胁迫码从裸 `SHA-256` 升级为 `scrypt-v1` 慢哈希记录
- 每次设置签到码 / 胁迫码都会生成独立随机 salt
- `scrypt` 参数支持通过 `SECRET_SCRYPT_N`、`SECRET_SCRYPT_R`、`SECRET_SCRYPT_P`、`SECRET_SCRYPT_MAXMEM` 配置
- 旧版 32 字节 `SHA-256` hash 不再兼容，升级后需要重新设置签到码 / 胁迫码
- 私钥、状态密钥、状态文件、HMAC、签到码 hash、胁迫码 hash 改为原子写入
- `keys/` 与 `vault/` 目录默认收紧为 `0700`
- 私钥、状态、HMAC 与口令 hash 默认写入为 `0600`

#### 验证与文档

- 新增 `tests/test_secret_hashing.py`，覆盖 `scrypt` 记录、拒绝旧 hash、敏感文件权限
- `.env.example` 增加慢哈希参数示例
- README 同步更新安全模型、配置说明、已知限制与更新记录
