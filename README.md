...
- 下载时才临时解密
- 下载后的明文临时文件在响应结束后删除
- 会话销毁时清理内存中的私钥引用并主动触发 `gc.collect()`
- Linux 下尝试对私钥材料执行 `mlock()`，降低交换到磁盘的概率
...
| 从运行中进程内存提取私钥 | 会话清理 + mlock 尝试 | ⚠️ 只能降低风险，不能彻底消除 |
...
- Linux 环境下尝试使用 `mlock()` 降低私钥交换到磁盘的风险
